...
function renderSalesProducts(showProduct) {
    ...
    document.getElementById('sales-products').innerHTML = productHtml;
}
...
function displaySalesProducts(productAll, perPage, currentPage) {
    let start = (currentPage - 1) * perPage;
    let end = (currentPage - 1) * perPage + perPage;
    let productShow = productAll.slice(start, end);
    renderSalesProducts(productShow);
}
...
function showSalesProduct(products) {
    let productAll = products.filter(item => item.status == 1)
    displaySalesProducts(productAll, perPage, currentPage);
    // setupPagination(productAll, perPage, currentPage);
}

window.onload = showHomeProduct(JSON.parse(localStorage.getItem('products')))
window.onload = showSalesProduct(JSON.parse(localStorage.getItem('sales-products')))
